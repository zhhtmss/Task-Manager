...
export function Dashboard() {
  ...
  const filterTasks = (task: Task, search: string, modes: SearchModes) => {
    ...
    const selectedModes = Object.values(modes).filter((mode) => mode).length;

    const searchEverywhere = selectedModes === 0 || selectedModes === 3;
  ...
  const currentTasks = tasks
    ...
    .filter((task) => filterTasks(task, currentSearch, currentModes));

  const archiveTasks = tasks
    .filter((task) => task.status !== "active")
    .filter((task) => filterTasks(task, archiveSearch, archiveModes));
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Проверь, запущен ли Express-сервер.
          </p>
        </div>
      </div>
    );
        ...
        >
          ...
          >
            <SearchBar
              ...
              onModeChange={(name) =>
                changeSearchMode(currentModes, setCurrentModes, name)
              }
            />
          </div>
        ...
        >
          ...
          >
            <div>
              ...
              >
                Archived tasks: {archiveTasks.length}
              </p>
            </div>
          </div>
          ...
          >
            <SearchBar
              ...
              onModeChange={(name) =>
                changeSearchMode(archiveModes, setArchiveModes, name)
              }
            />
          </div>
